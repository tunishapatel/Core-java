...
INNER JOIN employee ON employee.id = employee_salary.fk_employee_id ;
...
INNER JOIN hobby AS hb ON hb.id = eh.fk_hobby_id
WHERE emp.id = eh.fk_employee_id
)
FROM employee_salary AS es
INNER JOIN employee AS emp ON emp.id= es.fk_employee_id
GROUP BY emp.id;
